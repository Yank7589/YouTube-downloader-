...
bot = telebot.TeleBot('6903068807:AAHCxyONmNV11ddq6NrEoZ2S7oGFVJ0oJxI)
#@PY_87
print(' Go Bot /Start ')
...
def call(call):
    if call.data =='ty':
        ...
        mk = types.InlineKeyboardButton(text='قناة مطور لبوت',url='https://t.me/kotigori')
        ...
        mk = types.InlineKeyboardButton(text='قناة البوت',url='https://t.me/AlgerianQuran_bot')
        mk1 = types.InlineKeyboardMarkup(row_width=1)
        mk1.add(mk)
        message = bot.edit_message_text(chat_id=call.message.chat.id,message_id=call.message.message_id,text='*ارسل الان رابط المقطع من فضلك*',reply_markup=mk1,parse_mode='markdown')
...
def m1(message,id):
    ...
    if ('https') in me :
        #@PY_87
        #@PY_87
        ty = types.InlineKeyboardButton(text='مبرمج البوت',url='https://t.me/kotigori')
        kj = types.InlineKeyboardMarkup(keyboard=[[ty]])
